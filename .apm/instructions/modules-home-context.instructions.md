---
description: Reusable home-manager modules for AI tools (claude-code, codex, crush, hermes-agent, openclaw)
applyTo: "modules/home/**"
---
...
- Header pattern: `{ config, lib, pkgs, ... }: let inherit (lib) mkIf mkOption types; cfg = config.programs.<name>; in { options = {...}; config = mkIf cfg.enable {...}; }`
...
- Crush and hermes-agent are the most type-heavy; follow existing submodule shapes when adding MCP server types.
- `openclaw.nix` is an *extension* module — it adds options to the `programs.openclaw` namespace from the upstream nix-openclaw module without re-declaring options that upstream already owns.
