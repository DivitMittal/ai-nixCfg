...
| **Workflow / SDD** | `ralph-tui`, `openspec`, `openspecui`, `kestra`, `bead` (bd), `Beads-Viewer` (bv), `mardi-gras` |
...
{ inputs, ... }: {
  imports = [
    ...
    inputs.ai-nixCfg.homeManagerModules.kestra
...
| `kestra` | Workflow orchestration, run via podman (rootless OCI) against the upstream Kestra image in `server local` mode |
...
│   ├── default.nix             # Exports: ccs, codex, crush, herdr, kestra, ralph-tui, talon, ghnf
│   └── home/
...
│       ├── kestra.nix          # Kestra workflow orchestration (podman-run OCI image)
...
│
├── kestra/                     # Example Kestra flow definitions (deployed to a running server, not Nix-managed)
│   ├── README.md
│   └── flows/nixcfg-fleet-flake-update.yaml
│
